...
export default function adminStorage() {
  ...
  const addStorage = async () => {
    await axios.post("/api/Storage", form);
    fetchStorage();
  };

  const updateStorage = async (id) => {
    await axios.put(`/api/Storage?_id=${id}`, form);
    fetchStorage();
  };

  const deleteStorage = async (id) => {
    await axios.delete(`/api/Storage?id=${id}`);
    fetchStorage();
  };
  ...
  return (
    <Screen>
      ...
      <div className="p-4 m-6 rounded-lg">
        <p className="text-2xl font-bold mb-4 text-center">Storage</p>
          ...
          <tbody>
            {storage.map((item) => (
              <tr key={item._id} className="hover:bg-gray-100">
                ...
                <td className="py-2 px-4 border-b">{item.priceRange}</td>
                <td className="py-2 px-4 border-b">{item.stock}</td>
                <td className="py-2 px-4 border-b">{item.wattage}</td>
                <td className="py-2 px-4 border-b">{item.type}</td>
                <td className="py-2 px-4 border-b">{item.capacity}</td>
                <td className="py-2 px-4 border-b">
                  <button
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded mr-2"
                    onClick={() => updateStorage(item._id)}
                  >
                    Update
                  </button>
                  <button
                    className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded"
                    onClick={() => deleteStorage(item._id)}
                  >
                    Delete
                  </button>
                </td>
        ...
        <div className="mt-6">
          <h3 className="text-lg font-semibold mb-2">Add Storage</h3>
